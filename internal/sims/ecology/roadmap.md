...
* Latest tuning pass deepened crater spillways, increased lava tip mass transfer, and raised default reservoir/cooling extremes so scripted eruptions now push rivers ~80 tiles from the crater during regression sweeps, comfortably exceeding the volcano diameter goal.
...
* Lava parameter sweep harness now evaluates candidates in parallel so fallback tuning runs finish faster when defaults regress.
